 ...
 * IOSLanding
 *
 * Full marketing page for iOS visitors using the same light theme as Landing.tsx.
 * Uses the Aceternity UI Iphone component in the hero section.
 * Detects Android visitors and renders AndroidLanding instead.
 */

import React, { useEffect, useState, useRef, memo, useCallback, lazy, Suspense, Component, ReactNode } from 'react';
import {
...
const MOBILE_DEMO_SECTIONS = [
  {
    label: 'Harmony Score',
    title: 'Discover Your\nHome\'s Energy',
    desc: 'Every property has a unique energy signature. Our AI-powered Vastu analysis gives you a clear Harmony Score so you can make informed decisions about where to live.',
    highlights: [
  ...
  },
  {
    ...
    desc: 'No Vastu expertise needed. Ask any question about your home\'s energy and get practical, actionable guidance based on ancient principles.',
    highlights: [
  ...
  },
  {
    ...
    title: 'Scout Any\nNeighborhood',
    desc: 'Enter any ZIP code and instantly rank properties by their Harmony Score. Find the most Vastu-aligned homes without visiting each one.',
    highlights: [
  ...
  {
    label: 'Scan: Interior 3D',
    title: 'Scan Your\nInterior in 3D',
    desc: 'Coming soon to iPhone Pro. Use Apple\'s LiDAR sensor to create a 3D model of your interior and get room-by-room harmony insights.',
    highlights: [
      { icon: <Smartphone className="w-4 h-4" />, color: 'text-amber-500', text: 'iPhone Pro LiDAR scanning' },
      { icon: <Navigation className="w-4 h-4" />, color: 'text-purple-500', text: '3D interior modeling' },
      { icon: <Sparkles className="w-4 h-4" />, color: 'text-emerald-500', text: 'Room-by-room Vastu remedies' },
    ],
    cta: { text: 'Join the Scan Waitlist', href: '#ios-waitlist' },
  },
];
...
const HERO_SUBHEADLINES = [
  ...
  "LiDAR interior scanning coming to iPhone Pro.",
  "Discover properties aligned with nature's energy.",
];

// --- Bento demos (reuse from Landing) ---
...
const FAQ_ITEMS = [
  ...
  { question: 'What is Scan?', answer: "Scan is our upcoming iOS app that uses iPhone Pro's LiDAR sensor to create 3D interior models and provide room-by-room Vastu harmony analysis." },
    ...
    <nav className={`nav-fixed ${isScrolled ? 'nav-scrolled' : ''}`}>
      <div className="nav-container">
        ...
        <div className="md:hidden flex items-center gap-3">
          <button onClick={() => {
            document.getElementById('ios-waitlist')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
...
const IOSLanding: React.FC = () => {
  const [isAndroid] = useState(() => typeof window !== 'undefined' && /Android/i.test(window.navigator.userAgent));
  ...
  if (isAndroid) return <AndroidLanding />;

  return (
    <div className="page-root">
      <SEO
        title="Halo Home for iOS - Scan Your Space in 3D"
        description="Download Halo Home Scan for iPhone Pro. Use LiDAR to create 3D interior models and get room-by-room Vastu harmony analysis with actionable remedies."
        canonical="/ios"
      />
      ...
      <main>
        ...
        <div className="bg-section-hero relative overflow-hidden">
          ...
          <section className="ios-hero-section relative z-[1]">
            <div className="ios-hero-container">
                ...
                <div className="ios-hero-actions">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    <span className="text-zinc-500 text-xs">
                      ZIP Code Scout available now &mdash;{' '}
              ...
              {/* iPhone mockup with rotating Scan demo stages */}
            ...
            <ScrollReveal key={i}>
              <section className="px-6 py-14 max-w-lg mx-auto">
                ...
                <a href={section.cta.href}
                  onClick={section.cta.href === '#ios-waitlist' ? scrollToWaitlist : undefined}
        ...
        {/* ── FINAL CTA / WAITLIST ── */}
        <div className="bg-section-black section-block relative overflow-hidden">
          {/* Dot grid background */}
          <div className="absolute inset-0 cta-dot-grid" aria-hidden="true" />
          {/* LiDAR rings centered */}
          <div className="lidar-grid-bg cta-lidar-center" aria-hidden="true">
            <div className="lidar-ring lidar-ring-1" />
            <div className="lidar-ring lidar-ring-2" />
            <div className="lidar-ring lidar-ring-3" />
            <div className="lidar-scan-sweep" />
          </div>
          {/* Radial fade overlay to keep text readable */}
          <div className="absolute inset-0 bg-radial-fade pointer-events-none" aria-hidden="true" />
          <section id="ios-waitlist" className="section-wrapper max-w-3xl relative z-10">
            <ScrollReveal>
              <div className="flex flex-col items-center text-center">
                {/* Badge */}
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/15 bg-white/5 mb-6">
                  <Sparkles size={14} className="text-[#F0A6B3]" />
                  <span className="text-xs font-medium text-zinc-300">Coming Soon</span>
                </div>

                {/* Heading */}
                <h2 className="text-3xl md:text-5xl font-normal text-white leading-tight mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>
                  Be the First to Scan
                </h2>
                <p className="text-zinc-400 text-base md:text-lg max-w-lg mx-auto mb-8">
                  Join the waitlist and get notified the moment Halo Home launches on iPhone Pro & Android.
                </p>
                ...
                <div className="flex flex-wrap justify-center gap-2 mb-8">
                  {['LiDAR 3D Scanning', 'AI Vastu Analysis', 'Harmony Score', 'Room Insights'].map((f) => (
                    <div key={f} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
                ...
                {/* App badges — fixed height container for visual alignment */}
                <div className="flex items-center justify-center gap-3">
                  <a href="/ios" className="transition-transform hover:-translate-y-0.5" aria-label="Download on the App Store">
                    <AppStoreBadge height={44} width="auto" />
                  </a>
                  <GooglePlayBadge height={44} onClick={() => {
                    const el = document.getElementById('android-waitlist');
                ...
                <p className="text-sm text-zinc-500 mt-6">
                  Want ZIP code analysis now?{' '}
                  <a href="/guest" className="text-[#F0A6B3] font-semibold no-underline hover:text-[#e8939f]">Try Scout free &rarr;</a>
                </p>
              </div>
            </ScrollReveal>
          </section>
